        ...
        public class Handler : RequestHandlerBase<Request, Response>
        {
            public override Response Execute(Request message)
            {
                ...
                using (var transaction = session.BeginTransaction())
                {
                    ...
                    {
                        query = direction == SortDirection.Ascending
                            ? query.OrderBy(x => x.Order.Branch.Name)
                            : query.OrderByDescending(x => x.Order.Branch.Name);
                    });
                    ...
                    {
                        query = direction == SortDirection.Ascending
                            ? query.OrderBy(x => x.Order.InvoiceNumber)
                            : query.OrderByDescending(x => x.Order.InvoiceNumber);
                    });
                    ...
                    {
                        query = direction == SortDirection.Ascending
                            ? query.OrderBy(x => x.Order.Total.Amount)
                            : query.OrderByDescending(x => x.Order.Total.Amount);
                    });
